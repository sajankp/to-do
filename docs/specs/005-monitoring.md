...
- Custom metrics implemented: login/registration counters, todo create/complete/delete counters, AI request/latency/error metrics, `fasttodo_todos_per_user`, `fasttodo_db_connections_active`, `fasttodo_db_query_duration_seconds`.
- `fasttodo_ai_tokens_used_total` tracks audio message counts (input/output) as a proxy since Gemini Live API does not expose token counts in streaming mode.
